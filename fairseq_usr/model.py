...
from .transformer_prompt_decoder import TransformerDecoderPrompt, TransformerDecoderPromptBert
...
logger = logging.getLogger("fairseq_usr.model")
@dataclass
class Prompts:
    deep_prompt: bool
    prompt_length: int
...
class GSLMPromptConfig(TransformerLanguageModelConfig):
    ...
    linear_verbalizer: bool = field(
        default=True, metadata={"help": "if using linear verbalizer"}
    )
    num_classes: int = field(
        default=2, metadata={"help": "number of classes in classification task"}
    )
    use_sep_token: bool = field(
        default=True, metadata={"help": "if using sep token <s> "}
    )
    fine_tune: bool = field(
        default=True, metadata={"help": "if finetuning the whole model"}
    )
    verbalizer: Optional[Verbalizer] = None
    prompts: Optional[Prompts] = None
    
    # Bert
    use_nlp_embedding: bool = field(default=False, metadata={"help": "if using nlp embedding"})
    freeze_nlp_model: bool = field(default=True, metadata={"help": "whether to freeze nlp embedding"})
    nlp_model_name: str = field(default="bert-base-uncased", metadata={"help": "bert-base-uncased"})
    extractor_vocab_size: int = field(default=100, metadata={"help": "bert-base-uncased"})
...
class GSLMPromptModel(TransformerLanguageModel):
    ...
    def build_model(cls, args, task):
        ...
        if args.prompt_length > 0:
            ...
            prompts = Prompts(None, 0, None, None, None)
        ...
        if args.linear_verbalizer and not args.fine_tune:
            verbalizer = cls.build_linear_verbalizer(
                args, args.num_classes, task.source_dictionary
            )
        else:
            # verbalizer = Verbalizer(None, None)
            verbalizer = None
        if args.use_nlp_embedding:
            DECODER_CLASS = TransformerDecoderPromptBert
        else:
            DECODER_CLASS = TransformerDecoderPrompt
        ###############
        # Build Model #
        ###############
        ...
        logger.info(f"Fine-tune the decoder : {args.fine_tune}")
        logger.info(f"Fine-tune the decoder : {args.fine_tune}")
        logger.info(f"Fine-tune the decoder : {args.fine_tune}")
        if not args.fine_tune:
            ...
            for name, p in decoder.named_parameters():
                ...
                # add bert
                if "bert_proj" in name:
                    p.requires_grad = True
        else:
            logger.info("Fine-tune the decoder !!!")
            logger.info("Fine-tune the decoder !!!")
            logger.info("Fine-tune the decoder !!!")
    ...
    def build_linear_verbalizer(cls, args, num_classes, dictionary):
        ...
        """
        linear_verbalizer = torch.nn.Linear(
            len(dictionary), num_classes + 4, bias=False
        )
        return Verbalizer(num_classes, linear_verbalizer)

    @classmethod
    def build_prompt_embedding(
        cls, args, dec_embed_dim, decoder_layers, prompt_length, deep_prompt=False
    ):
        """
        ...
        # === input prompt === #
        dec_input_prompt = torch.nn.Embedding(
            prompt_length, dec_embed_dim, padding_idx=None
        )
        ...
        if deep_prompt:
            dec_key_prompt = ModuleList(
                [
                    torch.nn.Embedding(prompt_length, dec_embed_dim, padding_idx=None)
...
def GSLM_SpeechPrompt_v1(args):
    ...
    # May need to further modified, but currently I can't set fine_tune to True
    args.fine_tune = True
