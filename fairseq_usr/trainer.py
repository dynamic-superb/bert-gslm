...
class PromptTrainer(FairseqTrainer):
    ...
    def _save_filtered_params(self, state_dict: Dict, path):
        param_filters = global_config[
            "prompt_param_filter"
        ]  # ["sep", "prompt", "verbalizer"]
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        if self.cfg["model"]["fine_tune"]:
            ...
            self._save_base_model(
                state_dict, base_prompt_model_path
            ) if not base_prompt_model_path.is_file() else None
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                embed_weight = state["model"]["decoder.embed_tokens.weight"]
                projection_weight = state["model"]["decoder.output_projection.weight"]
                ...
                self.model.decoder.embed_tokens.weight.data[
                    : embed_weight.shape[0], :
                ] = embed_weight
                self.model.decoder.output_projection.weight.data[
                    : projection_weight.shape[0], :
                ] = projection_weight

                # bert_weight = torch.load(
                #     "/home/cyhuang/instruction-speech-prompt/new_weight.ckpt"
                # )
                text_lm = BertModel.from_pretrained("bert-base-uncased")
                text_emb = text_lm.embeddings.word_embeddings.weight
                self.model.decoder.embed_tokens.weight.data[
                    embed_weight.shape[0] :, : text_emb.shape[1]
                ] = text_emb
                self.model.decoder.output_projection.weight.data[
                    projection_weight.shape[0] :, : text_emb.shape[1]
                ] = text_emb
